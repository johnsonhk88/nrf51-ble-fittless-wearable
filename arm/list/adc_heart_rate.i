...
volatile int rate[16];                    
volatile int rate_max=0;
volatile int rate_min=0;              
volatile unsigned long sampleCounter = 0;          
volatile unsigned long lastBeatTime = 0;           
volatile int P =(3000>>1/3*1024)/1200;                      
volatile int T = (3000>>1/3*1024)/1200;                     
volatile int thresh = ((3000>>1/3*1024)/1200+15);                
volatile int Pulse_amp = 100;                   
	...
	
{
	  ...
	  ((NRF_ADC_Type *) 0x40007000UL)->CONFIG     = (ADC_res                                    << (0UL))     |    
                          ((0x02UL) << (2UL))  |    
                          ((0x00UL)                      << (5UL))  |   
                          (ADC_input_select                           << (8UL))    |
                          ((0UL)                  << (16UL));
...
void ADC_IRQHandler(void)  
{
    ...
    ((NRF_ADC_Type *) 0x40007000UL)->ENABLE     = (0x00UL);
}
void adc_start(void)  
{
...
uint16_t adc_Heart_read(void)  
{
  ...
  ((NRF_ADC_Type *) 0x40007000UL)->EVENTS_END = 0;
  adc_data = ((NRF_ADC_Type *) 0x40007000UL)->RESULT;
  return adc_data;
}
...
void adc_Heart_Signal_Process(void)
{
 ...
 Signal=  adc_result;  
 sampleCounter++;      
 Pulse_Timing = sampleCounter - lastBeatTime;  
...
#line 192 "..\\user\\adc_heart_rate.c"
        ...
        if(rate[i]>rate_max)
         ...
         runningTotal += rate[i]; 
			...
			runningTotal = runningTotal-rate_max-rate_min;   
			runningTotal /= 14;                     
    ...
    QS = 1;                              
       ...
       Pulse = 0;                         
       Pulse_amp = P - T;                 
       thresh = (Pulse_amp>>1) + T;                    
       P = thresh;                            
       T = thresh;                            
    } 
		
   if (Pulse_Timing > 2500)
		 {                           
     thresh = (3000>>1/3*1024)/1200+15;                          
     P = (3000>>1/3*1024)/1200;                               
     T = (3000>>1/3*1024)/1200;                               
     lastBeatTime = sampleCounter;          
     firstBeat = 1;                      
     secondBeat = 0;                    
     } 
