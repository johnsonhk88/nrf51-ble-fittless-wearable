...
static uint32_t                       m_active_low_states_mask;     
static uint32_t                       m_active_high_states_mask;    
...
static void button_handler_execute(app_button_cfg_t * p_btn)
{
    if (m_evt_schedule_func != 0)
    {
        ...
        do { const uint32_t LOCAL_ERR_CODE = (err_code); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler((LOCAL_ERR_CODE), 41, (uint8_t*) "..\\Source\\app_common\\app_button.c"); } while (0); } } while (0);
    }
    else
    {
...
static void detection_delay_timeout_handler(void * p_context)
{
    ...
    if (event_pins_mask != 0)
    {
        ...
        for (i = 0; i < m_button_count; i++)
        {
            ...
            if (((event_pins_mask & (1 << p_btn->pin_no)) != 0) && (p_btn->button_handler != 0))
            {
...
uint32_t app_button_is_pushed(uint8_t pin_no, _Bool * p_is_pushed)
{
    ...
    {
        *p_is_pushed = ((m_active_low_states_mask & pin_mask) ? 1 : 0);
    }
    else
    {
        *p_is_pushed = ((m_active_high_states_mask & pin_mask) ? 1 : 0);
    }
