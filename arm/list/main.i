...
#line 329 "..\\user\\main.c"
OLED_ShowString(0,16, (uint8_t*)"BT:DIS");
OLED_Refresh_Gram();						

	

	ADC_Heart_Rate_Init((0x02UL), (16UL), 1);
	HRM_Run=1;
...
#line 364 "..\\user\\main.c"
	if(QS==1)
	{
	 ...
	 OLED_Refresh_Gram();
		...
		temp_buffer[5]='p';
		temp_buffer[6]='m';
	 err_code = ble_nus_send_string(&m_nus, temp_buffer, 8 );      
   if (err_code != ((0x0) + 8))
   {
   do { const uint32_t LOCAL_ERR_CODE = (err_code); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler((LOCAL_ERR_CODE), 386, (uint8_t*) "..\\user\\main.c"); } while (0); } } while (0);
   }
