...
#define APP_TIMER_PRESCALER             0                                           //RTC1 分频器的值
...
static uint8_t                          BT_TR_time_count=0;                         //计时变量
...
/*************************************************
APPS UART communication

Requirement

 (base on nRF51 SDK_5_0_0 and softDevice_s110_6_0_0_API)
 ...
 void UART0_IRQHandler(void)
{
    ...
    index++;
    if (((data_array[index - 1] == '\n')&&(index==8)) || (index >= (BLE_NUS_MAX_DATA_LEN - 1)))
    {
        err_code = ble_nus_send_string(&m_nus, data_array, index );  //index+1
...
{
  NRF_TIMER2->MODE           = TIMER_MODE_MODE_Timer;        // 配置TIMER0工作在定时器模式
  NRF_TIMER2->PRESCALER      = 9;                            // 9预分频 Prescaler 9 produces 31250 Hz timer frequency => 1 tick = 32 us.
  NRF_TIMER2->BITMODE        = TIMER_BITMODE_BITMODE_16Bit;  // 16BIT计数模式
    
  
  NRF_TIMER2->CC[0]          = 31250;
  NRF_TIMER2->TASKS_START    = 1;                            // 启动定时器任务
	 
	NRF_TIMER2->INTENSET = TIMER_INTENSET_COMPARE0_Enabled << TIMER_INTENSET_COMPARE0_Pos;   //允许比较器0中断
	NRF_TIMER2->EVENTS_COMPARE[0]  = 0;                                                      //清除比较器0的事件
	NVIC_SetPriority(TIMER2_IRQn, 3);                                                        //设定定时器0中断的优先级别
  NVIC_EnableIRQ(TIMER2_IRQn);                                                             //使能定时器0的中断
	__enable_irq();                                                                          //使能全局中断
}
