...
//#include "ble_proximity.h"     // define all ble proximity apps 
...
void leds_init(void)
{
    ...
    GPIO_LED_CONFIG(ALERT_PIN_NO);
	  GPIO_LED_CONFIG(LED2);
	  GPIO_LED_CONFIG(LED3);
}
...
void gap_params_init(void)
{
    ...
    err_code = sd_ble_gap_device_name_set(&sec_mode, DEVICE_NAME, strlen(DEVICE_NAME)); //use SVCALL
    APP_ERROR_CHECK(err_code);
...
void ble_stack_init(void)
{
    BLE_STACK_HANDLER_INIT(NRF_CLOCK_LFCLKSRC_XTAL_20_PPM,
                           BLE_L2CAP_MTU_DEF,
                           ble_evt_dispatch,    // event_list
                           false);      // scheduler(false)
}
...
void bond_manager_init(void)
{
    ...
    // Initialize the Bond Manager (set band storge flash)
    ...
    err_code = ble_bondmngr_init(&bond_init_data);  // intial (load bonding list from flash)
    APP_ERROR_CHECK(err_code);
}
...
void buttons_init(void)
{
    static app_button_cfg_t buttons[] =
    {
        ...
        {STOP_ALERTING_BUTTON, false, NRF_GPIO_PIN_PULLUP, button_event_handler}   // using same handler
    };
